...
async def check_spam(message: Message):
    ...
    # --- ИЗМЕНЕННАЯ ЛОГИКА (РЕВЕРС-БЛОКИНГ) ---
    # Загружаем правила: ТОЛЬКО для темы, в которой отправлено сообщение
    ...
    if not words:
        ...
        return # <-- ВАЖНО: выходим, если нет правил для конкретной темы
