...
async def cmd_start(message: Message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        welcome_text = (
            ...
            "   Правила, добавленные для <code>topic_id = 0</code>, <u>работают ТОЛЬКО</u> в \"веб-ветке _1\" и НЕ действуют в других темах!\n\n"
...
async def callback_add_rule(callback: types.CallbackQuery):
    await callback.message.edit_text(
        ...
        "💡 Правила для <code>topic_id = 0</code> работают ТОЛЬКО в \"веб-ветке _1\" и НЕ действуют в других темах!",
        parse_mode="HTML",
        reply_markup=create_navigation_keyboard(None)
    )
    await callback.answer()
...
async def callback_help(callback: types.CallbackQuery):
    help_text = (
        ...
        "• <b>ВАЖНО:</b> <code>topic_id = 0</code> используется для \"веб-ветки _1\" и всей основной группы.\n"
        "• Правила, добавленные для <code>topic_id = 0</code>, НЕ действуют в других темах!\n\n"
        ...
        "• <code>topic_id</code> — это <u>числовой ID настоящей темы</u> (форума)\n"
        "• Бот учитывает регистр при проверке слов\n"
        "• Правила для <code>topic_id = 0</code> НЕ действуют в других темах!\n"
        "• Бот удаляет только сообщения за последние 48 часов"
    )
...
# --- Фоновая задача: сканирование истории сообщений ---
async def scan_recent_messages():
    """Фоновая задача: проверяет последние 10 сообщений в чате и удаляет спам."""
    while True:
        try:
            # Получаем последние 10 сообщений из основной группы (включая все темы)
            messages = await bot.get_chat_history(
                chat_id=-1002977868330,  # ← ЗАМЕНИТЕ НА ВАШ CHAT_ID, если нужно
                limit=10,
                offset=0
            )
            for msg in messages:
                ...
                topic_id = msg.message_thread_id  # может быть None (для веб-ветки _1)
                text_to_check = msg.text.lower()

                # Получаем правила для этой темы
                words = get_rules(-1002977868330, topic_id)
                if not words:
                    words = get_rules(-1002977868330, None)  # fallback на "веб-ветку _1"
                ...
                for word in words:
                    if word.lower() in text_to_check:
                        try:
                            ...
                            logging.info(f"✅ Удалено спам-сообщение ID={msg.message_id} по слову '{word}'")
                        except Exception as e:
                            logging.error(f"❌ Не удалось удалить {msg.message_id}: {e}")
                        break
        except Exception as e:
            logging.error(f"❌ Ошибка в scan_recent_messages: {e}")
...
async def check_spam(message: Message):
    ...
    # --- РЕВЕРС-БЛОКИНГ: только для конкретной темы ---
    words = get_rules(chat_id, topic_id)
    if not words:
        logging.info(f"ℹ️ Нет правил для этой темы (chat={chat_id}, topic={topic_id}). Сообщение не удаляется.")
        return
...
# --- ЗАПУСК ---
async def main():
    # Запускаем фоновую задачу сканирования
    asyncio.create_task(scan_recent_messages())
